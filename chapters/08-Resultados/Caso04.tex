...
\end{adjustbox}
\end{listing}
\paragraph{Salida} Registro detallado de 50 épocas (placeholders de log: \texttt{img/resultados/caso04\_logs.txt}). Destaca mejoras sucesivas de $\lambda$ hasta alcanzar $\lambda_{\text{best}} \approx -5.885$ en la época 9 y una reducción adicional a $\lambda_{\text{best}} \approx -6.012$ al final. También reporta eventos de reseeding cuando el radio local decrece de $0.0400$ a $0.0289$.
...
\end{adjustbox}
\end{listing}
\paragraph{Salida} Impresión: $\lambda_{\text{original}} = -2.805940$ y $\lambda_{\text{optimizado}} = -6.012074$, confirmando una reducción significativa de caos (placeholder textual).
...
\end{adjustbox}
\end{listing}
\paragraph{Salida} Figura interactiva (placeholder: \texttt{img/resultados/caso04\_lambda.png}) mostrando la convergencia hacia $\lambda \approx -6.0$ tras la época 9.
...
\end{adjustbox}
\end{listing}
\paragraph{Salida} Segunda figura (placeholder: \texttt{img/resultados/caso04\_fitness.png}) donde se observa el incremento del fitness después de la reselección en la época 8.
...
\end{adjustbox}
\end{listing}
\paragraph{Salida} Mensaje de confirmación, forma del tensor (\texttt{(17500, 3, 6)}) y última fila de la trayectoria (placeholder numérico para documentación). % chktex 36
...
\end{adjustbox}
\end{listing}
\paragraph{Salida} Logs (placeholder: \texttt{img/resultados/caso04\_summary.txt}) con energía total, pasos, intervalo temporal y distancias máximas/mínimas entre cuerpos.
...
\end{adjustbox}
\end{listing}
\paragraph{Salida} Figura mostrada in situ (placeholder: \texttt{img/resultados/caso04\_planar.png}) con las órbitas proyectadas en el plano.
...
\end{adjustbox}
\end{listing}
\paragraph{Salida} Objeto de animación renderizado en la célula (placeholder: \texttt{img/resultados/caso04\_anim3d.gif}) listo para exportarse a video.
...
\end{adjustbox}
\end{listing}
\paragraph{Salida} Archivo MP4 persistido (placeholder: \texttt{artifacts/caso04/trayectoria\_optima.mp4}), sin salida adicional.
...
\end{adjustbox}
\end{listing}
\paragraph{Salida} Tres figuras (placeholders: \texttt{img/resultados/caso04\_comparativa.png}, \texttt{img/resultados/caso04\_planos.png}, \texttt{img/resultados/caso04\_distancias.png}) y una animación secundaria si se renderiza con \texttt{HTML}.
...
\end{adjustbox}
\end{listing}
\paragraph{Salida} Archivo MP4 (placeholder: \texttt{artifacts/caso04/comparativa\_masas.mp4}); no imprime texto adicional.
...
\end{adjustbox}
\end{listing}
\paragraph{Salida} Texto con la ruta del CSV, tabla de las primeras 10 filas y tabla agregada (placeholder: \texttt{img/resultados/caso04\_timings.png}) donde destaca que \texttt{notebook\_run}, \texttt{batch\_eval} y \texttt{fitness\_eval} concentran la mayor duración.
...
\end{adjustbox}
\end{listing}
\paragraph{Salida} Mensajes del comando ejecutado y cuatro imágenes (placeholders: \texttt{img/resultados/timeline\_individual\_c04.png}, \texttt{img/resultados/timeline\_batch\_c04.png}, \texttt{img/resultados/timeline\_sim\_c04.png}, \texttt{img/resultados/pie\_sections\_c04.png}).
