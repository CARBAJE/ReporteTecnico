...
\paragraph{Descripción general}
\texttt{StreamingGA} envuelve la configuración del algoritmo genético de \texttt{pymoo} (\texttt{GA}) para el caso unidimensional de minimización (mLCE negativo). Expone una interfaz incremental (\texttt{ask}/\texttt{tell}) y se degrada a un sampler aleatorio con recombinación propia if \texttt{pymoo} no estó instalado o falla el \emph{setup}.
